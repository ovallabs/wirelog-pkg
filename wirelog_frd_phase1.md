...
2. Logging never blocks or fails a provider call. Enqueue is `select { case ch <- rec: default: dropped++ }`. Insert failures: one Logger line, drop the batch adding len(batch) to the drop counter, never propagate. A panicking user callback (Masker, PathNormalizer) must not fail the call either: the capture is recovered, abandoned, and counted as dropped. Dropped() reports total records that never reached the DB: enqueue drops plus insert-failure drops. RoundTrip returns the same *http.Response with only resp.Body swapped for a reader yielding identical bytes; the error return is always the wrapped transport's error, unmodified.
